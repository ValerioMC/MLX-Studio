...
/** The thread in the shape the conversation store saves: finished text turns only. */
export function toStoredMessages(
  thread: readonly UIMsg[],
): { role: UIMsg["role"]; content: string; tok_per_sec: number | null }[] {
  return thread
    .filter((m) => !m.streaming && !m.error && (m.role !== "assistant" || m.content.trim() !== ""))
    .map((m) => ({ role: m.role, content: m.content, tok_per_sec: m.tokPerSec ?? null }));
}
